...
from config.custom_permissions import OnlyLoggedUser
...
def news_detail(request, news):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            # Yangi koment obektini yaratamiz lekin db ga saqlamaymiz
            ...
            # db ga saqlaymiz
            new_comment.save()
            return redirect('news_detail_page', news.slug)
    else:
...
class HomePageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['xorijiy_xabarlar'] = News.published.all().filter(category__name='XORIJIY').order_by("-publish_time")[:5]
        context['texnologik_xabarlar'] = News.published.all().filter(category__name='TEXNOLOGIYA').order_by("-publish_time")[:5]
        context['foto_xabarlar'] = News.published.all().filter(category__name='FOTOXABARLAR').order_by("-publish_time")[:6]
...
class FotoNewsView(TemplateView):
    model = News
    template_name = 'news/foto-xabarlar.html'
    content_object_name = 'foto-xabarlari'

    def get_queryset(self):
        news = self.model.published.all().filter(category__name = 'FOTOXABARLAR')
        return news
    
class NewsUpdateView(OnlyLoggedUser, UpdateView):
    model = News
    fields = ['title', 'body', 'image', 'category', 'status']
    template_name = 'crud/news_edit.html'

class NewsDeleteView(OnlyLoggedUser, DeleteView):
    model = News
